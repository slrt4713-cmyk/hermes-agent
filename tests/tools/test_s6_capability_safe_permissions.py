...
def test_static_supervise_modes_are_set_before_ownership_changes() -> None:
    ...
    assert script.index(supervise_mode) < script.index(supervise_owner)
    assert script.index(event_mode) < script.index(event_owner)
